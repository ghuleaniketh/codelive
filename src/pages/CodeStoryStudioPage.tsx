...
import { createRealWorldStory, getActionSound, type RealWorldStory } from "@shared/types/learning/realWorldStory";
import { createCityRouteStory, createCityRouteWalkthrough, createDijkstraRouteWalkthrough, getCityGraphEdges, getCityGraphPositions, getCityLiveNarration, getCityRouteWalkthrough, getCityWeightedEdges, parseCityGraph, type CityGraph, type CityNodePosition, type CityRouteAlgorithm, type CityRouteState, type CityWeightedGraph } from "@/lib/learning/cityRoutes";
import { getStoryShortcutAction, STORY_SHORTCUTS } from "@/lib/learning/storyControls";
import { getStoryCodeLines } from "@/lib/learning/storyFocus";
import { getSavedVisualTheme, getVisualTheme, saveVisualTheme, visualThemes, type VisualTheme } from "@/lib/learning/learningThemes";
import { getInitialLearningWorkspace, getLearningWorkspace, getLearningWorkspaceLabel, type LearningWorkspace } from "@/lib/learning/workspaceNavigation";
import { defaultOnboardingStatus, finishOnboardingTour, getNextOnboardingStep, getPendingOnboardingWorkspace, getPreviousOnboardingStep, onboardingSteps, parseGraphScenario, readOnboardingStatus, serializeGraphScenario, shouldDisplayOnboardingCoach, type OnboardingWorkspace, type OnboardingStatus, type SharedGraphScenario } from "@/lib/learning/learningFlow";
import { createCityMapExportData, getCityMapExportFileBase } from "@/lib/learning/cityMapExports";
import { getStoryLearningScore } from "@/lib/learning/learningScore";
import { getExecutionTraceVisualState } from "@/lib/learning/executionTraceVisualState";
...
function normalizeExecutionResult(result: {
  status: "success" | "error" | "runner-unavailable";
  timeline?: { language: string; events: Array<Record<string, unknown>>; totalSteps: number };
  stdout?: string;
  stderr?: string;
  durationMs?: number;
}): ExecutionTimeline {
  const events: ExecutionEvent[] = (result.timeline?.events ?? []).map((event, index) => ({
    id: `${index + 1}`,
    sequence: index + 1,
    step: index + 1,
    line: typeof event.line === "number" ? event.line : 1,
    type: "line",
    data: {},
    explanation: typeof event.type === "string" ? event.type : undefined,
  }));
  const status = result.status === "success" ? "ready" : result.status;
  return {
    language: (result.timeline?.language ?? "javascript") as ExecutionTimeline["language"],
    source: "",
    mode: "runtime",
    status,
    events,
    output: result.stdout ? result.stdout.split("\n").filter(Boolean) : [],
    error: result.status === "error" || result.status === "runner-unavailable"
      ? { kind: result.status === "error" ? "runtime" : result.status, message: result.stderr || "The isolated runtime could not complete this request." }
      : undefined,
...
function ResultStageSignals({ variables, cells, pointers, syntaxNodes }: { variables: number; cells: number; pointers: number; syntaxNodes: number }) {
  const signals = [
    { x: 88, y: 48, tone: "cyan", label: "VALUES", value: variables },
    { x: 330, y: 34, tone: "violet", label: "CELLS", value: cells },
    { x: 570, y: 48, tone: "rose", label: "POINTERS", value: pointers },
    { x: 714, y: 70, tone: "amber", label: "AST", value: syntaxNodes },
  ];
  return <g className="simple-2d-stage-result-signals" aria-hidden="true">
    {signals.map((signal) => <g key={signal.label} className={`result-signal result-signal-${signal.tone}`} transform={`translate(${signal.x} ${signal.y})`}>
      <path className="result-signal-lead" d="M4 30 18 18" />
      <rect className="result-signal-card" width="78" height="28" rx="8" />
      <text className="result-signal-label" x="9" y="11">{signal.label}</text>
      <text className="result-signal-value" x="9" y="22">{signal.value}</text>
      <circle className="result-signal-pip" cx="66" cy="14" r="4" />
    </g>)}
  </g>;
}
...
function Simple2DVisualPanel({ step, previousStep, sourceLines, showBefore, showThreeResult, language, structure }: { step: LearningStep; previousStep?: LearningStep; sourceLines: string[]; showBefore: boolean; showThreeResult: boolean; language: Language; structure: CodeStructureSummary | null }) {
  ...
  return (
    <section className="simple-2d-diagram" data-primary-2d-scene data-state-view={showBefore ? "before" : "after"} aria-label={`2D visual for line ${shownStep.line}`}>
      ...
      <div className="simple-2d-visual-stage" data-react-svg-stage data-dimensional-svg-stage="isometric" data-expanded-result-stage="true" data-svg-icon-system="story-glyphs" data-selected-stage-node={selectedStageNode} data-result-scene={resultScene.variant}>
        <svg className="simple-2d-stage-svg" viewBox="0 0 960 248" preserveAspectRatio="none" aria-hidden="true">
          ...
          <g className="simple-2d-stage-data-cubes" aria-hidden="true">
            ...
            <ResultStageSignals variables={variables.length} cells={array?.cells.length ?? 0} pointers={pointers.length} syntaxNodes={structure?.nodeCount ?? 0} />
...
function formatMockTraceValue(value: unknown): string {
  return typeof value === "string" ? value : JSON.stringify(value) ?? String(value);
}

function TraceStatePanel({ event }: { event: ExecutionEvent }) {
  const visualState = getExecutionTraceVisualState(event);
  const { arrays, frames, pointers, recursiveFrames } = visualState;

  return (
    <section className="mt-3 grid gap-3 lg:grid-cols-[1.12fr_.88fr]" data-fixture-trace-visualization aria-label="Fixture trace state">
      <div className="rounded-xl border border-violet-300/25 bg-violet-50/80 p-3" data-fixture-trace-variables>
        <div className="flex items-center justify-between gap-2"><p className="text-[10px] font-black uppercase tracking-[0.12em] text-violet-900">Variables · trace state</p><span className="text-[10px] font-bold text-violet-700">{visualState.variables.length} tracked</span></div>
        {visualState.variables.length ? <div className="mt-2 grid gap-1.5 sm:grid-cols-2">{visualState.variables.map((variable) => <div key={variable.name} className="rounded-lg border border-violet-300/20 bg-white/80 px-2 py-1.5 font-mono text-[10px] text-slate-800"><strong>{variable.name}</strong><span className="ml-1 text-slate-600">= {formatMockTraceValue(variable.value)}</span></div>)}</div> : <p className="mt-2 text-[11px] text-slate-600">This source line has no trace variable state yet.</p>}
        <div className="mt-3 border-t border-violet-300/20 pt-3" data-fixture-trace-arrays><p className="text-[10px] font-black uppercase tracking-[0.12em] text-violet-900">Arrays and pointer arrows</p>{arrays.length ? arrays.map(([name, cells]) => <div key={name} className="mt-2 rounded-lg border border-sky-300/30 bg-sky-50/80 p-2"><div className="flex flex-wrap items-center gap-1"><strong className="mr-1 font-mono text-[10px] text-sky-950">{name}</strong>{cells.map((cell, index) => <span key={`${name}-${index}`} className="min-w-8 rounded border border-sky-300/35 bg-white px-1.5 py-1 text-center font-mono text-[10px] text-slate-800"><small className="mr-1 text-slate-500">{index}</small>{formatMockTraceValue(cell)}</span>)}</div>{pointers.filter((pointer) => pointer.array === name).map((pointer) => <p key={`${pointer.name}-${pointer.index}`} className="mt-1 text-[10px] font-bold text-sky-800">↓ {pointer.name} → {name}[{pointer.index}]</p>)}</div>) : <p className="mt-2 text-[11px] text-slate-600">No array-shaped state is present for this trace event.</p>}</div>
      </div>
      <div className="rounded-xl border border-amber-300/30 bg-amber-50/80 p-3" data-fixture-trace-call-stack>
        <div className="flex items-center justify-between gap-2"><p className="text-[10px] font-black uppercase tracking-[0.12em] text-amber-900">Function-call stack · fixture</p><span className="text-[10px] font-bold text-amber-700">{frames.length} frames</span></div>
        <ol className="mt-2 space-y-1.5">{frames.map((frame, index) => <li key={`${frame.name}-${index}`} className={`rounded-lg border px-2.5 py-2 text-[11px] ${/recursive/i.test(frame.name) ? "border-fuchsia-300/50 bg-fuchsia-100 text-fuchsia-950" : "border-amber-300/25 bg-white/80 text-slate-800"}`}><span className="mr-2 inline-grid h-4 w-4 place-items-center rounded-full bg-amber-200 text-[9px] font-black text-amber-950">{index + 1}</span><strong>{frame.name}</strong><span className="ml-1 text-slate-500">line {frame.line}</span></li>)}</ol>
        {recursiveFrames.length ? <p className="mt-3 rounded-lg border border-fuchsia-300/40 bg-fuchsia-100 px-2.5 py-2 text-[11px] font-semibold text-fuchsia-950" data-fixture-trace-recursion>Recursion is shown here as a fixture nested frame. It is not a measured runtime call stack.</p> : <p className="mt-3 text-[11px] text-slate-600">Fixture frames are contract data, not runtime frames.</p>}
      </div>
    </section>
  );
}
...
export default function Home() {
  ...
  const projects = { isLoading: false, isError: false, data: [] as Array<{ id: number; title: string; language: Language; code: string }> };
  const [activeProjectId, setActiveProjectId] = useState<number | null>(null);
  const { theme, preference, setPreference } = useTheme();
  const saveSubmission = trpc.submissions.create.useMutation({
  ...
  const runIsolatedExecution = trpc.execution.run.useMutation();
  const runnerState = trpc.execution.getRunnerState.useQuery();
  ...
  const execution = useExecution(executionTimeline ?? undefined);
  const executionEvent = execution.currentEvent;
  ...
  useEffect(() => {
    if (activeView !== "studio" || !steps.length) return;
    const scroller = explanationScrollRef.current;
    const activeEntry = scroller?.querySelector<HTMLElement>('[data-story-explanation="true"]');
    if (!scroller || !activeEntry) return;

    const prefersReducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    scroller.scrollTo({
      top: Math.max(0, activeEntry.offsetTop - 8),
  ...
  const startVisualStory = async () => {
    ...
    if (selectedWalkthrough) {
      ...
      try {
        const submission = await saveSubmission.mutateAsync({ code: userCode, language: selectedLang });
        const apiStory = await interpretStory.mutateAsync({ submissionId: submission.id });
        const sourceLines = userCode.split("\n");
        generatedSteps = apiStory.steps.map((story, index) => ({
          step: index + 1,
          line: index + 1,
          code: sourceLines[index] ?? "",
          story: createRealWorldStory(story.text, index + 1),
        }));
        setCurrentStructure(null);
      } catch {
  ...
  const saveCurrentProject = () => {
    if (!isAuthenticated) {
      toast.info("Sign in to save private projects.");
      startLogin();
      return;
    }
    if (!userCode.trim()) {
      toast.error("Add code before saving a project.");
      return;
    }
    toast.info("Project storage is not available yet. Your code remains in the editor.");
  };

  const openProject = (project: { id: number; title: string; language: Language; code: string }) => {
    setActiveProjectId(project.id);
    setUserProblem(project.title);
    setSelectedLang(project.language);
    setUserCode(project.code);
    setSelectedWalkthrough(null);
    setExecutionTimeline(null);
    toast.success(`Opened ${project.title}.`);
  };

  const startExecutionPreview = async () => {
    if (!userCode.trim()) {
      toast.error("Please paste or write some code first.");
      return;
    }
    ...
    try {
      toast.info("Source timeline preview is not available yet. Your visual story is still available.");
    } catch {
      toast.error("The source timeline preview could not be created. Your visual story is still available.");
    }
  };

  const startIsolatedExecution = async () => {
    if (!runnerState.data?.enabled) {
      toast.info("Runtime execution is disabled until the private Judge0 runner is configured and verified.");
      return;
    }
    if (!userCode.trim()) {
      toast.error("Please paste or write some code first.");
    ...
    try {
      const submission = await saveSubmission.mutateAsync({ code: userCode, language: selectedLang });
      const result = await runIsolatedExecution.mutateAsync({ submissionId: submission.id });
      const timeline = normalizeExecutionResult(result);
      setExecutionTimeline(timeline);
      execution.reset();
      if (timeline.status === "ready") {
        toast.success("Isolated runtime output was captured.");
      } else if (timeline.status === "error") {
        toast.error(timeline.error?.message || timeline.output.join("\n") || "The isolated runtime could not complete this request.");
      } else if (timeline.status === "runner-unavailable") {
        toast.error("The private Judge0 runner is unavailable right now. Please try again in a moment.");
      } else {
        toast.error("The isolated runtime could not complete this request.");
      }
    } catch {
      toast.error("The isolated runtime request could not be completed safely.");
    }
  };
  ...
  const startCityComparison = (sharedScenario?: SharedGraphScenario) => {
    try {
      ...
      saveSubmission.mutate({ language: "python", code: generatedCode });
    ...
    <div className={`lab-shell visual-theme-${visualTheme} min-h-screen overflow-x-hidden`} data-visual-theme={visualTheme} data-appearance={theme} data-mandala-intensity={mandalaIntensity} data-mandala-sound={mandalaSoundsEnabled ? "on" : "off"} style={{ "--mandala-user-accent": mandalaAccent } as React.CSSProperties}>
              ...
              <div data-code-only className="relative flex flex-wrap items-center justify-between gap-4 border-b border-white/10 pb-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs" style={{ color: "#091320" }}>Use JavaScript, Python, C, C++, or Java. A problem title is optional.</p>
                  </div>
                </div>
                <div data-language-selector className="flex rounded-2xl border border-[#3c2b20] bg-[#0c0806] p-1">
                  {(["javascript", "python", "c", "cpp", "java"] as Language[]).map((language) => <button key={language} onClick={() => setSelectedLang(language)} className={`rounded-xl px-3 py-2 text-[10px] font-black uppercase tracking-wide transition ${selectedLang === language ? "bg-gradient-to-r from-[#ffbd7d] to-[#d86527] text-[#1a0c05] shadow-[0_0_18px_rgba(229,155,99,0.25)]" : "text-[#a89787] hover:text-white"}`}>{language === "cpp" ? "C++" : language}</button>)}
                </div>
              </div>
              <div className="relative mt-6 space-y-5">
                <div>
                  <div data-code-only data-code-field-heading className="mb-2 flex items-center justify-between gap-3"><Label className="block text-[11px] font-bold uppercase tracking-wider text-[#bba797]">What does your code do? <span className="font-normal normal-case tracking-normal text-[#8f7c6d]">Optional</span></Label><span className="text-[10px] text-[#8f7c6d]">A short sentence helps your story title.</span></div>
                ...
                <div data-code-only><div data-code-field-heading className="mb-2 flex items-center justify-between gap-3"><Label className="block text-[11px] font-bold uppercase tracking-wider text-[#bba797]">Your code</Label><span className="text-[10px] text-[#8f7c6d]">Line-aware editor · Tab indents · Every non-empty line becomes one visual step.</span></div><Textarea data-code-input value={userCode} onChange={(event) => { setUserCode(event.target.value); setSelectedWalkthrough(null); }} onKeyDown={(event) => { if (event.key !== "Tab") return; event.preventDefault(); const field = event.currentTarget; const start = field.selectionStart; const end = field.selectionEnd; setUserCode(`${userCode.slice(0, start)}  ${userCode.slice(end)}`); requestAnimationFrame(() => { field.selectionStart = field.selectionEnd = start + 2; }); }} rows={16} className="resize-y rounded-xl border-[#503525] bg-[#0b0705] p-4 font-mono text-xs leading-6 text-[#f7f0e8] shadow-[inset_0_0_24px_rgba(0,0,0,0.24)] focus-visible:ring-[#f59e0b]" placeholder="Paste your code here" /></div>
                ...
                <section data-code-only className="mt-4 rounded-2xl border border-emerald-300/20 bg-emerald-300/[0.055] p-4" aria-label="Private projects">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div><p className="text-[10px] font-black uppercase tracking-[0.14em] text-emerald-800">Private projects</p><p className="mt-1 text-xs font-semibold text-slate-800">{isAuthenticated ? `Signed in as ${user?.email ?? "learner"}` : "Sign in to save and reopen private projects."}</p></div>
                    <div className="flex gap-2"><Button type="button" variant="outline" onClick={saveCurrentProject} disabled={authLoading} className="h-9 border-emerald-300/30 text-xs font-black text-emerald-950">{activeProjectId ? "Update project" : "Save project"}</Button></div>
                  </div>
                  {isAuthenticated && <div className="mt-3 flex flex-wrap gap-2">{projects.isLoading ? <span className="text-xs text-slate-600">Loading your projects…</span> : projects.isError ? <span className="text-xs font-semibold text-rose-700">Project storage is temporarily unavailable. Your editor remains usable.</span> : projects.data?.length ? projects.data.map((project) => <button type="button" key={project.id} onClick={() => openProject(project as { id: number; title: string; language: Language; code: string })} className={`rounded-lg border px-2.5 py-2 text-left text-[11px] font-bold transition ${activeProjectId === project.id ? "border-emerald-400 bg-emerald-100 text-emerald-950" : "border-slate-200 bg-white text-slate-700 hover:border-emerald-300"}`}><span className="block max-w-40 truncate">{project.title}</span><span className="mt-0.5 block text-[9px] uppercase tracking-wide opacity-70">{project.language === "cpp" ? "C++" : project.language}</span></button>) : <span className="text-xs text-slate-600">No saved projects yet.</span>}</div>}
                </section>
                <section data-execution-preview className="mt-4 rounded-2xl border border-sky-300/20 bg-sky-300/[0.055] p-4" aria-label="Safe execution timeline preview">
                  <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.14em] text-sky-800">Execution foundation</p><h3 className="mt-1 text-sm font-black text-slate-950">Study a source preview while runtime stays disabled.</h3><p className="mt-1 max-w-2xl text-xs leading-5 text-slate-700">This source preview reads lines and snapshots likely state changes. It does not compile, run, or evaluate your code.</p><p data-runner-state className="mt-2 text-[10px] font-bold uppercase tracking-[0.12em] text-slate-600">{runnerState.data?.enabled ? "Private Judge0 runtime ready" : "Private Judge0 runtime disabled until server configuration is verified"}</p></div><div className="flex flex-wrap gap-2"><Button type="button" data-create-execution-preview onClick={startExecutionPreview} className="h-10 rounded-xl bg-sky-300 px-4 text-xs font-black text-slate-950 hover:bg-sky-200 disabled:opacity-60">Preview timeline</Button><Button type="button" variant="outline" data-run-isolated-execution onClick={startIsolatedExecution} disabled={!runnerState.data?.enabled || runIsolatedExecution.isPending} className="h-10 rounded-xl border-slate-300 px-4 text-xs font-black text-slate-800 disabled:opacity-50">{runIsolatedExecution.isPending ? "Running safely…" : "Run in private sandbox"}</Button></div></div>
                  {executionTimeline && <div className="mt-4 rounded-xl border border-sky-300/20 bg-white/70 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-black text-slate-900">{executionTimeline.mode === "preview" ? "Source preview · no runtime execution" : executionTimeline.mode === "fixture" ? "Fixture trace · test data only" : "Runtime timeline"}</p><span className="text-[10px] font-bold uppercase tracking-wide text-slate-600">{executionTimeline.events.length} events</span></div>{executionTimeline.mode === "fixture" && <p className="mt-2 rounded-lg border border-amber-300/30 bg-amber-50 px-2.5 py-2 text-[11px] font-semibold text-amber-950" data-fixture-trace-disclaimer>{executionTimeline.fixture?.disclaimer ?? "Fixture trace: deterministic test data only."}</p>}{executionEvent ? <><p className="mt-2 text-xs font-bold text-slate-800">Line {executionEvent.line} · {executionEvent.type}</p><p className="mt-1 text-xs leading-5 text-slate-700">{executionEvent.explanation}</p><div className="mt-3 flex flex-wrap items-center gap-2"><Button type="button" variant="outline" size="sm" onClick={execution.previous} disabled={execution.step === 0}>Previous</Button><Button type="button" variant="outline" size="sm" onClick={execution.togglePlay} disabled={execution.step >= executionTimeline.events.length - 1}>{execution.isPlaying ? "Pause" : "Play"}</Button><Button type="button" variant="outline" size="sm" onClick={execution.next} disabled={execution.step >= executionTimeline.events.length - 1}>Next</Button><Button type="button" variant="outline" size="sm" onClick={execution.reset}>Reset</Button><span className="text-[10px] font-bold text-slate-600">Event {execution.step + 1} / {executionTimeline.events.length}</span></div><TraceStatePanel event={executionEvent} /></> : <p className="mt-2 text-xs text-slate-700">{executionTimeline.error?.message ?? "No source events are available."}</p>}</div>}
                </section>
            </div>
      ...
      {activeView === "studio" && currentStep && (
        <main className="lab-grid studio-learning-frame mx-auto w-full max-w-7xl px-5 py-5 md:px-8" data-dense-learning-workspace>
          ...
          <section className="execution-workspace-grid simple-story-page code-visual-explanation-linear grid gap-3 xl:grid-cols-[14.5rem_minmax(0,1fr)_19rem]" data-simple-story-page data-learning-linear-layout="code-visual-explanation">
            <aside className="execution-rail simple-code-panel lab-surface h-fit rounded-[20px] p-3.5 md:p-4" data-execution-rail data-learning-stair-step="code" aria-label="Current code execution">
            ...
            <div className="lab-surface story-stage primary-visual-stage simple-visual-panel rounded-[20px] p-3.5 md:p-4" data-primary-visual-stage data-learning-stair-step="visual">
              ...
              <section className="persistent-step-explanation mt-3" data-persistent-explanation aria-hidden="true">
            ...
            <aside className="lab-surface story-explanation-panel direct-explanation-panel simple-explanation-panel rounded-[20px] p-3.5 md:p-4" data-direct-explanation-panel data-learning-stair-step="explanation">
              ...
              <div ref={explanationScrollRef} className="simple-explanation-scroll mt-2.5" data-explanation-scroll data-active-explanation-step={currentStepIndex + 1} aria-label="Explanation history">
                {steps.slice(0, currentStepIndex + 1).map((step, index) => { const isActive = index === currentStepIndex; return <button type="button" key={`explanation-history-${step.step}-${step.line}-${index}`} onClick={() => goToStep(index)} aria-current={isActive ? "step" : undefined} aria-label={`Go to explanation step ${index + 1}, line ${step.line}`} data-explanation-entry={index + 1} {...(isActive ? { "data-story-explanation": true, "data-explanation-quality": true, "aria-live": "polite" } : {})} className={`simple-explanation-copy ${isActive ? "is-current explanation-enter" : ""}`}><div className="simple-explanation-entry-meta"><span>Step {index + 1}</span><span>Line {step.line}</span></div><p>{step.story.plainEnglish}</p></button>; })}
              </div>
            </aside>
          </section>
        </main>
